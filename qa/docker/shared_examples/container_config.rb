shared_examples_for 'it runs with different configurations' do |flavor|
  ...
  context 'when a single pipeline is configured via volume bind' do
    # let(:options) { {"HostConfig" => { "Ulimits" => [{"Name" => "nofile", "Soft"=> 65535, "Hard" => 65535 }, {"Name" => "noproc", "Soft"=> 65535, "Hard" => 65535 }],
    #                                     "Binds" => ["#{FIXTURES_DIR}/simple_pipeline/:/usr/share/logstash/pipeline/"] } } }
    let(:options) { {"HostConfig" => {
                                       "Binds" => ["#{FIXTURES_DIR}/simple_pipeline/:/usr/share/logstash/pipeline/"] } } }
